		...
		)
{
	...
	_dim = (unsigned long)(_nx+2*_xh) * (unsigned long)(_ny+2*_yh) * (unsigned long)(_nz+_sky_z+2*_zh) * (unsigned long)4;
	...
	_icalc = new Index_Calculator(_Target_Platform,_Kernel_Type==0?1:2,_nx,_ny,_nz,_sky_z,_xh,_yh,_zh);
...
{
	_Create_Compressed_Earth_Model_No_Q(OTflag,gamfac);
	...
	for (int iZ = 0;  iZ < _nz;  ++iZ)
	{
		for (int iX = 0;  iX < _nx;  ++iX)
		{
			...
			else if (_Kernel_Type == 1)
			{
				...
				float C44C33 = Attribute_Loader::Decompress_Attribute(VelAnis,0.0f,1.0f,C44C33MASK,SHIFTC44C33);
				if (Vp_fp != 0L) fprintf(Vp_fp, "%d %d %e\n",iX,iZ,Vp);
...
void Acoustic_Earth_Model::Check_Earth_Model_Parameters()
{
	if (
			_vp_Is_Constant < 0 ||
			_den_Is_Constant < 0 ||
			_eps_or_eta_Is_Constant < 0 ||
			_dta_Is_Constant < 0 ||
			_dip_or_dx_Is_Constant < 0 ||
			_azm_or_dy_Is_Constant < 0 ||
			_Q_Is_Constant < 0 ||
			_dimx < 1 ||
			_dimy < 1 ||
			_dimz < 1
	...
	{
		_Valid_Earth_Model_Parameters = _ucalc->Is_Valid();
	}
}
...
void Acoustic_Earth_Model::_Add_Q_To_Earth_Model(float fq)
{
	...
	{
		Add_Q(_PadVelAnis,"Loading Q",_Q_File,_Q_Constant,_Q_range,_Q_binsize,QMASK,SHIFTQ,_fq,_dt);
	}
	else // VTI and TTI
	{
		Add_Q(_PadDenAng,"Loading Q",_Q_File,_Q_Constant,_Q_range,_Q_binsize,QMASK,SHIFTQ,_fq,_dt);
	}
}
		...
		)
{
	...
	Scan_Earth_Model(OTflag,gamfac);
	...
	else if (_Target_Platform == 1) // GPU
	{
		...
		if (_Kernel_Type > 0)
		{
			cudaError_t err1 = cudaHostAlloc((void**)&_PadVelAnis, _dim * (long)2, cudaHostAllocDefault);
			_PadDenAng = _PadVelAnis + _icalc->Get_Field_Offset();
			if (err1 != cudaSuccess) return;
			omp_memset(_PadVelAnis, _dim * (long)2);
		}
		else
		{
