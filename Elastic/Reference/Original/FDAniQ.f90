...
integer :: i, j, k, ijk, ir, kti, l, isnps, lr, li  
integer :: ii, jj, kk
integer :: ijk_dec
integer :: sti, nsti
integer :: idum
...
character (len=1) :: rectype ! rec type 
...
open(unit=1, file='FDAniQ.in', status='old', action='read', &
   & iostat=status)
! ..read grid info.:	
...
read(1,*)
read(1,*)
write(*,*) '..reading nx, ny, nz, dx, dy, dz'
read(1,*)  nx, ny, nz, dx, dy, dz  ! Earth model dimensions & grid size
...
read(1,*)
read(1,*) nrec, rectype, iline, xline

allocate(xrec(nrec), yrec(nrec), zrec(nrec))

write(*,*) '..reading xrec(li), yrec(li), zrec(li)'
read(1,*)
do i=1,nrec
  read(1,*) xrec(i), yrec(i), zrec(i)
enddo
...
write(*,*) '..reading xsou, ysou, zsou, soutype, souamp'
read(1,*)
do k=1,nsou
  ! sou loc., sou type [=p,f,v,m], sou amp, sou wavelet#  
  read(1,*) xsou(k), ysou(k), zsou(k), soutype(k)
  backspace(1)    ! now go back to read in source amplitude info.
  if (soutype(k) == 'p') then
    read(1,*)  dum, dum, dum, cdum, souamp(k,1)  ! pressure sou 
  elseif (soutype(k) == 'm') then
    read(1,*)  dum, dum, dum, cdum, (souamp(k,i), i=1,6)  ! mIJ sou
  else
    read(1,*)  dum, dum, dum, cdum, (souamp(k,i), i=1,3)  ! v or f sou
  endif
enddo                                                        
...
! ..rad fast-axis flag for flat-binary model files:
...
do kti = 1, nti    
  ...
  do sti = 1, nsti
   ...
   !!!!
    ...
    if (kti <= nwav) then
      do l=1, nsou
        if (soutype(l) == 'p') then  ! pressure sou
          ! pressure source: 
          ! ..nearest grid point: 
          icell = nint(xsou(l)/dx) + 1 ! to left of extrap pt
          jcell = nint(ysou(l)/dy) + 1 ! 
          kcell = nint(zsou(l)/dz) + 1 ! above extrap pt
  ...
  if (kti > ntshift) then  ! start storing trace data 
    ...
    do lr = 1, nrec
      do irectype = 1, 4  ! loop over rec type
        ...
        icell = nint(xrec(lr)/dx) + 1 
        jcell = nint(yrec(lr)/dy) + 1 
        kcell = nint(zrec(lr)/dz) + 1 
        ! proper fractional distances for pr rec (irectype == 1):
        dx_frac = (xrec(lr) - dx*float(icell - 1))/dx
        dy_frac = (yrec(lr) - dy*float(jcell - 1))/dy  
        dz_frac = (zrec(lr) - dz*float(kcell - 1))/dz  
        ...
        enddo
        if (irectype == 1) then
          prec(kti_shift,lr) = f_out
        elseif (irectype == 2) then
          vxrec(kti_shift,lr) = f_out
        elseif (irectype == 3) then
          vyrec(kti_shift,lr) = f_out
        elseif (irectype == 4) then
          vzrec(kti_shift,lr) = f_out
        endif
        ...
        if (ghostrecflag) then  
          ...
          if (irectype == 1) then
            ! neg mirror on pr
            prec(kti_shift,lr) = prec(kti_shift,lr) - f_out  
          elseif (irectype == 2) then
            ! neg mirror on vx
            vxrec(kti_shift,lr) = vxrec(kti_shift,lr) - f_out  
          elseif (irectype == 3) then
            ! neg mirror on vy (!!!!check this!)
            vyrec(kti_shift,lr) = vyrec(kti_shift,lr) - f_out  
          elseif (irectype == 4) then
            ! pos mirror on vz
            vzrec(kti_shift,lr) = vzrec(kti_shift,lr) + f_out  
          endif
